...
const ScrollFollowImage = () => {
  ...
  useEffect(() => {
    ...
    const missionCenter = missionRect.top + missionRect.height / 2;

    const heroBottom = heroRect.bottom;
    const missionTop = missionRect.top;

    if (scrollY < missionTop - window.innerHeight / 2) {
      setOffset(0);
    } else if (scrollY > missionTop) {
      const progress = Math.min(
        1,
        (scrollY - (missionTop - window.innerHeight / 2)) /
          (window.innerHeight / 2)
      );
      setOffset(progress * 300);
    } else {
      const progress = (scrollY - heroBottom + window.innerHeight) / window.innerHeight;
  ...
  return (
    <div
      ref={heroRef}
      className="relative group hidden lg:flex flex-col items-center justify-center"
      style={{
        transform: `translateY(${offset}px)`,
        transition: "transform 0.1s linear",
      }}
    >
      <div className="absolute inset-0 bg-gradient-to-br from-accent/30 via-accent/10 to-transparent rounded-full blur-3xl animate-pulse"></div>
      <div className="absolute -inset-12 bg-gradient-to-tr from-accent/20 to-transparent rounded-full blur-3xl opacity-60 animate-bounce"></div>
      <img
        src="/img3.png"
        alt="Our Philosophy - Product"
        className="relative w-64 h-80 object-contain drop-shadow-2xl animate-float transition-transform duration-200 group-hover:scale-105"
      />
      
      {/* Artistic CBC Watermark */}
      <div className="relative mt-8 flex flex-col items-center gap-3">
        <div className="flex items-center gap-2">
          <div className="w-1 h-8 bg-gradient-to-b from-accent to-transparent rounded-full"></div>
          <div className="text-center">
            <p className="text-accent font-bold tracking-[0.3em] text-sm uppercase opacity-60 hover:opacity-100 transition-opacity duration-300">
              CBC
            </p>
            <div className="w-12 h-0.5 bg-gradient-to-r from-transparent via-accent to-transparent mt-1 mx-auto"></div>
          </div>
          <div className="w-1 h-8 bg-gradient-to-b from-accent to-transparent rounded-full"></div>
        </div>
        <p className="text-accent/40 text-xs tracking-wider uppercase font-light">
...
const Philosophy = () => {
  ...
  return (
    <div className="w-full flex flex-col min-h-screen bg-primary">
      ...
      <section className="px-10 bg-primary h-[600px] flex items-center">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                <p className="text-secondery/70 text-sm leading-relaxed">
            ...
            <div className="relative group hidden lg:flex items-center justify-center">
              <div className="absolute inset-0 bg-gradient-to-br from-accent/20 via-accent/10 to-transparent rounded-3xl blur-2xl animate-pulse"></div>
              <div className="absolute -inset-8 bg-gradient-to-tr from-accent/30 to-transparent rounded-full blur-3xl opacity-50 animate-bounce"></div>
              <img
                src="/CBC_pistic.png"
                alt="Philosophy in action - product"
                className="relative w-64 h-96 object-contain drop-shadow-2xl animate-float transition-transform duration-500 group-hover:scale-110"
              />
            </div>
          </div>
        </div>
      ...
      <section className="h-[600px] px-10 bg-secondery relative overflow-hidden flex items-center">
        ...
        <div className="max-w-6xl mx-auto relative z-10 w-full">
          <div className="text-center space-y-4 mb-8">
            ...
            <p className="text-primary/70 text-sm max-w-3xl mx-auto">
...
const CommitmentItem = ({ number, label, description }) => {
  return (
    <div className="p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-sm hover:bg-white/10 hover:border-accent/30 transition-all duration-300 group">
      <div className="text-4xl font-bold text-accent mb-2 group-hover:text-white transition-colors duration-300">
        {number}
      </div>
